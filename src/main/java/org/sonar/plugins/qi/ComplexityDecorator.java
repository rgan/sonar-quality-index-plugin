...
public class ComplexityDecorator extends AbstractDecorator {
    ...
    private static final int CC_DISTRIB_FIRST_KEY = 2;
    private static final int CC_DISTRIB_SECOND_KEY = 10;
    private static final int CC_DISTRIB_THIRD_KEY = 20;
    private static final int CC_DISTRIB_FOURTH_KEY = 30;
    ...
    private double getMethodCount(DecoratorContext context, boolean weighted) {
        ...
        if (weighted) {
            methodWithComplexityCount = distribution.get(CC_DISTRIB_FIRST_KEY) + 3 * distribution.get(CC_DISTRIB_SECOND_KEY) +
                    5 * distribution.get(CC_DISTRIB_THIRD_KEY) + 10 * distribution.get(CC_DISTRIB_FOURTH_KEY);
        } else {
            methodWithComplexityCount = distribution.get(CC_DISTRIB_FIRST_KEY) + distribution.get(CC_DISTRIB_SECOND_KEY)
                    + distribution.get(CC_DISTRIB_THIRD_KEY) + distribution.get(CC_DISTRIB_FOURTH_KEY);
        }
    ...
    private double getComplexMethods(DecoratorContext context) {
        ...
        Map<Integer, Integer> distribution = KeyValueFormat.parse(measure.getData(), new KeyValueFormat.IntegerNumbersPairTransformer());
        return distribution.get(CC_DISTRIB_FOURTH_KEY);
    }
